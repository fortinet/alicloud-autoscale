...
async function makeDistAliCloudFgtASG() {
    console.info('Making distribution zip package');
    let pm = Packman.spawn(),
        rTempDir = await pm.makeTempDir(),
        rTempDirSrc = path.resolve(rTempDir, 'src'),
        rTempDirSrcFuncApp = path.resolve(rTempDirSrc, 'alicloud_function_compute_temp'),
        rTempDirSrcLib = path.resolve(rTempDirSrcFuncApp, './'),
        rTempDirSrcCore = path.resolve(rTempDirSrcLib, 'core'),
        rTempDirSrcAli = path.resolve(rTempDirSrcLib, './'),
        packageInfo,
        zipFilePath,
        rDirSrcCore = path.resolve(REAL_PROJECT_ROOT, './core'),
        rDirSrcAli = path.resolve(REAL_PROJECT_ROOT, './alicloud'),
        rDirSrcFuncCompute = path.resolve(REAL_PROJECT_ROOT, './alicloud_function_compute'),
        rDirDist = path.resolve(REAL_PROJECT_ROOT, './dist'),
        zipFileName,
        saveAsFile;
    ...
    await pm.makeDir(rTempDirSrc);
    await pm.makeDir(rDirDist);
    await pm.copyAndDelete(rDirSrcFuncCompute, rTempDirSrcFuncApp, ['node_modules', 'local', 'test',
        '.nyc_output', '.vscode', 'bin', 'obj',
        '*.csproj', 'proxies.json', 'host.json', 'local.settings.json', 'package-lock.json']);
    await pm.makeDir(rTempDirSrcLib);
    await pm.copyAndDelete(rDirSrcCore, rTempDirSrcCore,
        ['node_modules', 'local', 'test', '.nyc_output', '.vscode', 'package-lock.json']);
    await pm.copyAndDelete(rDirSrcAli, rTempDirSrcAli,
        ['node_modules', 'local', 'test', '.nyc_output', '.vscode', 'package-lock.json']);
    await pm.npmInstallAt(rTempDirSrcFuncApp,
        ['--save', rTempDirSrcAli.replace(rTempDirSrcFuncApp, '.')], {noSymlink: true});
...
switch (scrptName.toLowerCase()) {
    ...
    case 'project':
        makeDistProject();
        break;
    case 'alicloud-asg-deployment':
        makeDistAliCloudFgtASG();
        break;
    case 'all':
        makeDistAll();
        break;
